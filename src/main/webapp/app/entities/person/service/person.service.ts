...
export class PersonService {
  ...
  protected convertDateFromClient(person: IPerson): IPerson {
    return Object.assign({}, person, {
      dateOfBirth: person.dateOfBirth?.isValid() ? person.dateOfBirth.toJSON() : undefined,
      dateOfDeath: person.dateOfDeath?.isValid() ? person.dateOfDeath.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((person: IPerson) => {
        person.dateOfBirth = person.dateOfBirth ? dayjs(person.dateOfBirth) : undefined;
        person.dateOfDeath = person.dateOfDeath ? dayjs(person.dateOfDeath) : undefined;
      });
    }
    return res;
  }
